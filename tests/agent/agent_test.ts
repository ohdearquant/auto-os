...
Deno.test("Agent System", async (t) => {
  await t.step("Agent Creation and Configuration", async () => {
    ...
    await assertRejects(
      () =>
        Promise.resolve(
          new TestAgent(createMockAgentConfig({ name: "" })),
        ),
      ValidationError,
  ...
  await t.step("Agent State", async () => {
    ...
    const messagePromise = agent1.receiveMessage(
